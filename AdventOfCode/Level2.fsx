...
let input = loadLines "level2.txt"
...
let dimensions = 
    ...
    |> Seq.map 
       (fun s -> 
            let x::y::z::[] = s.Split('x') |> Seq.map Convert.ToInt32 |> Seq.toList
            x,y,z)

let wrapping (x,y,z) = 
    let sides = [x*y; x*z; y*z]
    let minSide = List.min sides
    minSide + (2 * Seq.sum sides)
...
let ribbon (x,y,z) =
    let bow = x * y * z
    let loop = [x;y;z] |> Seq.sort |> Seq.take 2 |> Seq.map ((*) 2) |> Seq.sum
    bow + loop
